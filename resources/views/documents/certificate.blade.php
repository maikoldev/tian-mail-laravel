...
<body>
    <header class="text-center">
        ...
        <p>
            Empresa autorizada para realizar capacitación en manipulación de alimentos por la Resolución No DPA.<br>
    ...
    <section class="name text-center">
        ...
        <p class="fullname text-blue">{{ mb_strtoupper($data['name']) }} {{ mb_strtoupper($data['lastname']) }}</p>
        <p class="document">con {{ $data['document_type'] }} No. <strong>{{ $data['document_number'] }}</strong></p>
    </section>

    <section class="texts">
        <p>
            Cursó y aprobó la capacitación de Manipulación de los Alimentos, Inocuidad y Buenas Prácticas de Manufactura
            con una intensidad de 10 horas, la cual tiene una vigencia de 12 meses a partir de la fecha de emisión del
            presente documento.
        </p>
        <br>
        <p>
            Los temas abordados en la presente certificación se distribuyen en 8 módulos, los cuales se alinean al plan
            de capacitación continuo y permanente en Prácticas Higiénicas en Manipulación de Alimentos, Buenas Prácticas
            de Manufactura y educación sanitaria, contemplado en los Artículos 12 y 13 de la Resolución 2674 de 2013 y
    ...
    <section class="signature text-center">
        <p>
            La presente certificación se expide cumpliendo con la normatividad vigente y bajo la firma autorizada por
    ...
    <table class="number-certificate">
        <tr>
            <td>
                ...
                <p><strong>{{ $data['certificate_number'] }}</strong></p>
            </td>
            <td>
                <p>Fecha de expedición</p>
                <p><strong>{{ $data['date_text'] }}</strong></p>
            </td>
        </tr>
    </table>
        ...
        <div class="wrapper">
            <div class="col-6">
                <div class="front">
                    <img
                        class="background"
                        src="images/carnet-front.jpeg"
                        width="280"
                    >
                    <img
                        class="avatar"
                        src="{{ public_path($data['photo_url']) }}"
                    >
                    <div class="wrap">
                        <p class="name">
                            <strong>
                                {{ mb_strtoupper($data['name']) }} {{ mb_strtoupper($data['lastname']) }}
                            </strong>
                        </p>
                        <p class="id field">{{ $data['document_number'] }}</p>
                        <p class="valid field">{{ $data['date_expiration'] }}</p>
                        <p class="serial field">{{ $data['certificate_number'] }}</p>
                    </div>
                </div>
            </div>
            <div class="col-6">
